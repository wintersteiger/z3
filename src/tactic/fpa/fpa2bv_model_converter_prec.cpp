...
Module Name:
    ...
    fpa2bv_model_converter_prec.h

Abstract:

    Model conversion for fpa2bv_converter with precision control.

Author:

    Christoph (cwinter) 2015-06-15
...
void fpa2bv_model_converter_prec::display(std::ostream & out) {
...
void fpa2bv_model_converter_prec::convert(model * bv_mdl, model * float_mdl) {
    fpa_util fu(m);
    bv_util bu(m);
    mpf fp_val, sfp_val;
         ...
         it++)
    {
        ...
        unsigned sfpa_ebits = fu.get_ebits(a->get_decl()->get_range());
        unsigned sfpa_sbits = fu.get_sbits(a->get_decl()->get_range());
        ...
        exp_unbiased_q = exp_q - fu.fm().m_powers2.m1(sfpa_ebits - 1);
        ...
        fu.fm().set(sfp_val, sfpa_ebits, sfpa_sbits, !mpqm.is_zero(sgn_q.to_mpq()), sig_z, exp_z);
        fu.fm().set(fp_val, ebits, sbits, MPF_ROUND_TOWARD_ZERO, sfp_val);
...
model_converter * mk_fpa2bv_model_converter_prec(ast_manager & m,
    ...
    return alloc(fpa2bv_model_converter_prec, m, const2bv, rm_const2bv, uf2bvuf, uf23bvuf);
}
